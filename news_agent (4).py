...
def daily_job(send_message: bool = False) -> None:
    ...
    if not articles:
        ...
        return
    # Break the articles into thematic categories and prepare separate messages for
    # each group.  Sending multiple shorter messages helps avoid Twilio's 1600
    # character limit per message.
    categories = categorize_articles(articles)
    messages: List[str] = []
    for topic, topic_articles in categories.items():
        # Skip empty categories
        if not topic_articles:
            continue
        # Limit to the most recent 5 articles per category and restrict overall length
        try:
            body = prepare_news_message(
                topic_articles,
            ...
            logging.error(f"Failed to build message for {topic}: {exc}")
            continue
        # Prepend a thematic label to differentiate the categories
        header = f"Notícias de {topic}:"
        full_message = f"{header}\n\n{body}"
        messages.append(full_message)
    if not messages:
        logging.warning("No messages to send after categorisation.")
        return
    logging.info(f"Prepared {len(messages)} message(s) for dispatch.")
    if send_message:
        for msg in messages:
            try:
                send_whatsapp_message(msg)
                # Sleep briefly between messages to avoid hitting rate limits
                time.sleep(1)
            except Exception as exc:
                logging.error(f"Failed to send WhatsApp message: {exc}")
    else:
        # When in test mode, log each message separately
        for idx, msg in enumerate(messages, start=1):
            logging.info(f"Message {idx} preview:\n{msg}\n")
