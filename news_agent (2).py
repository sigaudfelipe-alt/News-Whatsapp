...
def prepare_news_message(articles: List[Dict[str, str]], tz: str = "America/Sao_Paulo") -> str:
    ...
    lines: List[str] = []
    lines.append(f"Principais notícias de {today_str} (Economia & Política):")
    for idx, art in enumerate(articles, start=1):
        ...
        # Truncate long summaries to avoid exceeding WhatsApp message limits
        if summary_text:
            truncated = summary_text
            # Limit summary to roughly 250 characters or first two sentences
            if len(truncated) > 300:
                # Try to cut at the end of the second sentence if present
        ...
        if truncated:
            line_parts.append(f"– {truncated}")
        if source:
            line_parts.append(f"(Fonte: {source})")
        if link:
            line_parts.append(f"\nLink: {link}")
...
def daily_job(send_message: bool = False) -> None:
    ...
    try:
        message = prepare_news_message(articles, tz="America/Sao_Paulo")
    except Exception as exc:
